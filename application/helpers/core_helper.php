...
function getAdvancedSearch($class=''){
	$CI =getCI();
	//include('form-search.php');
	$CI->load->view($CI->jCfg['theme'].'/'.$class.'-search');
} 
...
function _encrypt($data = ""){
  ...
  $cipher = "AES-256-CBC";
  // Generate IV secara acak dengan panjang sesuai algoritma
  $iv = openssl_random_pseudo_bytes(openssl_cipher_iv_length($cipher));
  // Enkripsi data
  $encrypted = openssl_encrypt($data, $cipher, $key, 0, $iv);
  // Menggabungkan IV dan data terenkripsi (IV harus disimpan bersama data terenkripsi)
  $encrypted_with_iv = base64_encode($iv . $encrypted);

  $encoded_resut = rawurlencode($encrypted_with_iv);

  return $encoded_resut;
}

function _decrypt($encrypted_data=""){
	$CI =getCI();
	// if( cfg('mycript') == true ){
	// 	$CI->load->library('encrypt');
	...
	// 		return $nid_arr[1];
	// 	}else{
	// 		redirect($CI->own_link."?msg=Access Denied&type_msg=danger");
	// 	}
	// }else{
	// 	return $key;
	// }

  $decoded_encrypted_data = rawurldecode($encrypted_data);

  $key = "bWhjc29zMjAyNA";
  // Menggunakan AES-256-CBC untuk dekripsi
  $cipher = "AES-256-CBC";
  // Decode data dari base64
  $data = base64_decode($decoded_encrypted_data);
  // Panjang IV yang digunakan
  $iv_length = openssl_cipher_iv_length(cipher_algo: $cipher);
...
?>
		<style type="text/css">
		<!--
		.dataTables_length { padding: 5px; }
		.dataTables_filter { padding: 5px; width: 100%; }
		.dataTables_info { padding: 5px; }
...
function _get_menu($menu = array()){
  $CI = getCI();
  if (!$menu || count($menu) < 0) return array();
  $mnn = array();

  foreach ($menu as $mn) {
      $mnx = preg_split("/>/", $mn['acc_menu']);
      $count = count($mnx);
      $temp = &$mnn;  // Referensi ke $mnn
      ...
      for ($i = 0; $i < $count; $i++) {
          if ($i == $count - 1) {
              ...
              $temp[] = array(
                  'menu' => $mnx[$i],
                  'id' => $mn['acc_id'],
                  'class_group' => $mn['acc_group'],
                  'group' => $mn['acc_group_controller'],
                  'name' => $mn['acc_controller_name'],
                  'css_class' => $mn['acc_css_class']
              );
          } else {
              if (!isset($temp[$mnx[$i]])) {
                  $temp[$mnx[$i]] = array();
...
function option_province($prov=""){
	...
	if( count($q) > 0 ){
		foreach ($q as $key => $value) {
			$selected = $value->propinsi_nama==$prov?"selected='selected'":"";
			$callback .= "<option value='".$value->propinsi_nama."' $selected >".$value->propinsi_nama."</option>";
		}
	}
	return $callback;		
}

function option_kab($city=""){
	$CI  = getCI();
	
	$CI->db->order_by('kab_nama','ASC');
	$q = $CI->db->get("app_kabupaten")->result();

	if( count($q) > 0 ){
		foreach ($q as $key => $value) {
			$selected = $value->kab_nama==$city?"selected='selected'":"";
			$callback .= "<option value='".$value->kab_nama."' $selected >".$value->kab_nama."</option>";
		}
	}
	return $callback;		
}
...
function get_site_info($web_url=""){
	$CI = getCI();
	$return = array();
	...
	if(trim($web_url)!=""){
		$site = $CI->db->get_where("app_site",array(
				"site_url"	=> $web_url
			))->row();
//		debugCode($site);
		$return['site'] = $site;
		$template = $CI->db->get_where("app_site_template",array(
				"template_site_id"	=> $site->site_id
			))->row();
...
function option_domains($dom=""){
	$CI  = getCI();
	$CI->db->order_by('site_name','ASC');
	$CI->db->where('site_status',1);
	if( $CI->jCfg['user']['is_all'] != 1 ){
		$CI->db->where("site_id",$CI->jCfg['user']['domain']['id']);
	}
	$q = $CI->db->get("app_site")->result();
	$callback = "";
	if( count($q) > 0 ){
		foreach ($q as $key => $value) {
			$selected = $value->site_id==$dom?"selected='selected'":"";
			$callback .= "<option value='".$value->site_id."' $selected >".$value->site_name."</option>";
		}
	}
	return $callback;		
}

function option_client($cli=""){
	$CI  = getCI();
	$CI->db->order_by('client_name','ASC');
	$CI->db->where('client_status',1);
//	if( $CI->jCfg['user']['is_all'] != 1 ){
//		$CI->db->where("client_site_id",$CI->jCfg['user']['domain']['id']);
//	}
	$q = $CI->db->get("sos_client")->result();
	$callback = "";
	if( count($q) > 0 ){
		foreach ($q as $key => $value) {
			$selected = $value->client_id==$cli?"selected='selected'":"";
			$callback .= "<option value='".$value->client_id."' $selected >".$value->client_name."</option>";
		}
	}
	return $callback;		
}

function option_provider($id=""){
	$CI  = getCI();
	$CI->db->order_by('provider_name','ASC');
	$CI->db->where('provider_status',1);
//	if( $CI->jCfg['user']['is_all'] != 1 ){
//		$CI->db->where("provider_site_id",$CI->jCfg['user']['domain']['id']);
//	}
	$q = $CI->db->get("sos_provider")->result();
	$callback = "";
	if( count($q) > 0 ){
		foreach ($q as $key => $value) {
			$selected = $value->provider_id==$id?"selected='selected'":"";
			$callback .= "<option value='".$value->provider_id."' $selected >".$value->provider_name."</option>";
		}
	}
	return $callback;		
}

function option_providertype($type=""){
	$CI  = getCI();
	$CI->db->order_by('protype_type','ASC');
	$CI->db->where('protype_status',1);
	if( $CI->jCfg['user']['is_all'] != 1 ){
		$CI->db->where("protype_site_id",$CI->jCfg['user']['domain']['id']);
	}
	$q = $CI->db->get("sos_ref_provider_type")->result();
	$callback = "";
	if( count($q) > 0 ){
		foreach ($q as $key => $value) {
			$selected = $value->protype_type==$type?"selected='selected'":"";
			$callback .= "<option value='".$value->protype_type."' $selected > [ ".$value->protype_type." ] ".$value->protype_description."</option>";
		}
	}
	return $callback;		
}

function option_plantype($id=""){
	$CI  = getCI();
	$CI->db->order_by('plan_id','ASC');
	$CI->db->where('plan_status',1);
	if( $CI->jCfg['user']['is_all'] != 1 ){
		$CI->db->where("plan_site_id",$CI->jCfg['user']['domain']['id']);
	}
	$q = $CI->db->get("sos_ref_plan_type")->result();
	$callback = "";
	if( count($q) > 0 ){
		foreach ($q as $key => $value) {
			$selected = $value->plan_id==$id?"selected='selected'":"";
			$callback .= "<option value='".$value->plan_id."' $selected >[ ".$value->plan_type." ] ".$value->plan_description."</option>";
		}
	}
	return $callback;		
}

function option_servicetype($id="",$cid=""){
	$CI  = getCI();
	$CI->db->order_by('service_number','ASC');
	$CI->db->where('service_status',1);
	if( !empty($cid) ){
		$CI->db->where("service_clientid",$cid);
	}
//	if( $CI->jCfg['user']['is_all'] != 1 ){
//		$CI->db->where("service_site_id",$CI->jCfg['user']['domain']['id']);
//	}
	$q = $CI->db->get("sos_ref_service_type")->result();
	$callback = "";
	if( count($q) > 0 ){
		foreach ($q as $key => $value) {
			$selected = $value->service_id==$id?"selected='selected'":"";
			$callback .= "<option value='".$value->service_id."' $selected >".$value->service_type."</option>";
		}
	}
	return $callback;		
}

function option_relationshiptype_on_search($relate=""){
	$callback = '';
	$callback .= '<option value="employee"'.($relate=='employee'?' selected="selected"':'').'>Employee</option>';
	$callback .= '<option value="dependent"'.($relate=='dependent'?' selected="selected"':'').'>Dependent (Tertanggung)</option>';
	
	return $callback;
}

function option_relationshiptype($relate=""){
	$CI  = getCI();
	$CI->db->order_by('relationship_id','ASC');
	$CI->db->where('relationship_status',1);
	if( $CI->jCfg['user']['is_all'] != 1 ){
		$CI->db->where("relationship_site_id",$CI->jCfg['user']['domain']['id']);
	}
	$q = $CI->db->get("sos_ref_relationship_type")->result();
	$callback = "";
	if( count($q) > 0 ){
		foreach ($q as $key => $value) {
			$selected = $value->relationship_type==$relate?"selected='selected'":"";
			$callback .= "<option value='".$value->relationship_type."' $selected >".$value->relationship_type."</option>";
		}
	}
	return $callback;		
}

function option_employmentstatus($relate=""){
	$CI  = getCI();
	$CI->db->order_by('employment_id','ASC');
	$CI->db->where('employment_status',1);
	if( $CI->jCfg['user']['is_all'] != 1 ){
		$CI->db->where("employment_site_id",$CI->jCfg['user']['domain']['id']);
	}
	$q = $CI->db->get("sos_ref_employment_status")->result();
	$callback = "";
	if( count($q) > 0 ){
		foreach ($q as $key => $value) {
			$selected = $value->employment_activated==$relate?"selected='selected'":"";
			$callback .= "<option value='".$value->employment_activated."' $selected >".$value->employment_activated."</option>";
		}
	}
	return $callback;		
}
