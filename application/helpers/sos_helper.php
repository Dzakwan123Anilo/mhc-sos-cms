...
function getTransactions() {
  ...
  $query = "
    SELECT date_claim, b.service_type
    FROM sos_ttrans_claim a 
    INNER JOIN sos_ref_service_type b on a.servicetypeId = b.service_id AND a.clientId = b.service_clientid
    WHERE b.service_type IN ('Rawat Inap', 'Rawat Jalan', 'Emergency');
  ";
	...
	$q = $CI->db->query("
		SELECT * FROM (
			SELECT 'Rawat Inap' AS layanan, 1 AS nourut,
				sum(case when date_format(date_claim,'%y%u') = date_format(now(),'%y%u') then 1 else 0 end) as weeks,
				sum(case when date_format(date_claim,'%y%m') = date_format(now(),'%y%m') then 1 else 0 end) as months,
				sum(case when date_format(date_claim,'%y') = date_format(now(),'%y') then 1 else 0 end) as years
			FROM sos_ttrans_claim a
			INNER JOIN sos_ref_service_type b on a.servicetypeId = b.service_id AND a.clientId = b.service_clientid
			WHERE b.service_type LIKE '%Rawat Inap%'
			UNION
			...
			FROM sos_ttrans_claim a
			INNER JOIN sos_ref_service_type b on a.servicetypeId = b.service_id AND a.clientId = b.service_clientid
			WHERE b.service_type LIKE '%Rawat Jalan%'
			UNION
			...
			FROM sos_ttrans_claim a
			INNER JOIN sos_ref_service_type b on a.servicetypeId = b.service_id AND a.clientId = b.service_clientid
			WHERE b.service_type LIKE '%Rawat Gigi%'
			UNION
			...
			FROM sos_ttrans_claim a
			INNER JOIN sos_ref_service_type b on a.servicetypeId = b.service_id AND a.clientId = b.service_clientid
			WHERE b.service_type LIKE '%Emergency%'
  ...
  */
	...
	/*
		SELECT * FROM (
			...
			FROM sos_ttrans_claim a
			INNER JOIN sos_ref_service_type b on a.servicetypeId = b.service_id AND a.clientId = b.service_clientid
			WHERE b.service_description LIKE '%Rawat Inap%'
			UNION
			SELECT 'Rawat Jalan' AS layanan, 2 AS nourut, 
				sum(case when date_format(date_claim,'%y%u') = date_format(now(),'%y%u') then 1 else 0 end) as weeks,
				sum(case when date_format(date_claim,'%y%m') = date_format(now(),'%y%m') then 1 else 0 end) as months,
			...
			FROM sos_ttrans_claim a
			INNER JOIN sos_ref_service_type b on a.servicetypeId = b.service_id AND a.clientId = b.service_clientid
			WHERE b.service_description LIKE '%Rawat Jalan%'
			UNION
...
function getNumberActiveMember($param=array()) {
	...
	// $date = count($param) > 0 
  //         ? (empty($param['fmonth']) ? substr($param['period'],0,4) . date('-m-d') : $param['period'] . date('-d')) 
  //         : date('Y-m-d');
  // $query = "
  //   SELECT client_id, client_name, '".(empty($param['fmonth'])?"Previous Month":get_month(str_replace("-","",substr($param['period'],-2))-1))."' as showit, '".date('Y-n-j', strtotime(date('Y-n-j', strtotime($param['period'].'-'.date('j')))." -1 month"))."' as gen_date, '1' as nourut, 
  //   member_cardNo, m.time_add, member_statuscard
  ...
  // $query = "
  // SELECT client_id, client_name, '".(empty($param['fmonth'])?"Current Month":get_month(str_replace("-","",substr($param['period'],-2))))."' as showit, '".date('Y-n-j', strtotime($param['period'].'-'.date('j')))."' as gen_date, '2' as nourut, 
  //   member_cardNo, m.time_add, member_statuscard
  //   FROM sos_member AS m
  //   RIGHT JOIN sos_client AS c ON m.member_clientid = c.client_id
  ...
  foreach ($periods as $idx => $period) {
    ...
    $gen_date = date('Y-n-j', strtotime($period.'-'.date('j')));
    if ($period != date('Y-m')) $gen_date = date('Y-n-t', strtotime($period.'-'.date('j')));
    
    $whereClientId = empty($clientid)
                     ? ''
                     : " AND m.member_clientid = $clientid ";

    $query .= " 
    SELECT client_id, client_name, '".get_month($month)."' as showit, '".$gen_date."' as gen_date, $nourut as nourut, 
      sum(case when right(member_cardNo,1) = 0 AND date_format(m.time_add,'%y%m') <= date_format('$date','%y%m') AND member_statuscard = 'Active' then 1 else 0 end) as main_insured,
      sum(case when right(member_cardNo,1) != 0 AND date_format(m.time_add,'%y%m') <= date_format('$date','%y%m') AND member_statuscard = 'Active' then 1 else 0 end) as dependent
    FROM sos_member AS m
      RIGHT JOIN sos_client AS c ON m.member_clientid = c.client_id
    WHERE member_status = 1 AND c.client_status = 1 $whereClientId
      GROUP BY client_name 
    ";
  ...
  $query .= " ) AS x ORDER BY client_name, nourut ASC";
...
function getSingleNumberActiveMember($param=array()) {
	...
	$showit = empty($param['fmonth'])?get_period($param['row']):get_month($param['month']);
	$where = " AND member_status != 999";
  ...
  ";
	...
	$q = $CI->db->query("
		...
		FROM sos_member AS m
			RIGHT JOIN sos_client AS c ON m.member_clientid = c.client_id
		WHERE member_status = 1 AND c.client_status = 1 $where
			GROUP BY client_name
	")->row();
...
function getNumberSwipeByClient($param=array()) {
	...
	// 		SELECT client_id, client_name, 'Today' as showit, '".date('Y-n-j', strtotime($param['period'].'-'.date('j')))."' as gen_date, '3' as nourut,
  ...
  foreach ($periods as $idx => $period) {
    ...
    $query .= "
        SELECT client_id, client_name, '".get_month($month)."' as showit, '".date('Y-n-j', strtotime($period.'-'.date('j')))."' as gen_date, $nourut as nourut,
        ...
        FROM sos_ttrans_claim AS tc
          RIGHT JOIN sos_client AS c ON tc.clientId = c.client_id
          INNER JOIN sos_member AS m ON tc.memberId = m.member_id AND member_status = 1
          WHERE c.client_status = 1 $whereClientId
          GROUP BY client_name";
...
function getNumberMemberSwipeMoreThanOnce($param=array()) {
	...
	// $date = count($param)>0?(empty($param['fmonth'])?substr($param['period'],0,4).date('-m-d'):$param['period'].date('-d')):date('Y-m-d');
	// $today = empty($param['fmonth'])?"UNION
	// 		SELECT c.client_id, c.client_name, 'Today' as showit, '".date('Y-n-j', strtotime($param['period'].'-'.date('j')))."' as gen_date, '3' as nourut,
...
//	,date_format(createdDate,'%y%m%d')
  ...
  foreach ($periods as $idx => $period) {
    ...
    $query .= "
        SELECT c.client_id, c.client_name, '".get_month($month)."' as showit, '".date('Y-n-j', strtotime($period.'-'.date('j')))."' as gen_date, $nourut as nourut,
...
function getSingleMemberSwipeMoreThanOnce($param=array()) {
	...
	$where = !empty($param['clientid'])?" WHERE client_id = ".$param['clientid']:"";
	$q = $CI->db->query("
		SELECT c.client_id, c.client_name, '$showit' as showit, 
...
function getSingleNumberSwipeByProvider($param=array()) {
  ...
  foreach ($periods as $idx => $period) {
    ...
    $showit = get_month($month);
    $case = "AND date_format(date_claim,'%y%m') = date_format('$date','%y%m')";
    ...
    $query .= "
      SELECT provider_id, provider_name, '$showit' as showit, '$gen_date' as gen_date,
        sum(case when servicetypeId = 1 $case then 1 else 0 end) as inpatient,
        sum(case when servicetypeId = 2 $case then 1 else 0 end) as outpatient,
        sum(case when servicetypeId = 3 $case then 1 else 0 end) as dental,
        sum(case when servicetypeId = 4 $case then 1 else 0 end) as emergency,
  ...
  $query .= " ) AS x ORDER BY provider_name ASC";
...
function getNumberUserLogin($param=array()) {
	...
	$where = isset($param['providerid'])&&!empty($param['providerid'])?" AND p.provider_id = ".$param['providerid']:"";
	$q = $CI->db->query("
    ...
    WHERE log_date between date_add(now(),INTERVAL -2 MONTH) and now() $where
		GROUP BY provider_name
		ORDER BY provider_name ASC
	")->result();
...
function getExportSwipeByClient($p=array(),$count=FALSE) {
  ...
  $date = $p['date'];
	$date = " AND date_format(tc.date_claim,'%y%m') = date_format('$date','%y%m')";
	$limit = "";
