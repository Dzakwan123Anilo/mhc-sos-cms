...
// debugCode(count($this->jCfg['user']['role']));
		if( count($this->jCfg['user']['role']) > 0){
			...
			$menu = $this->db->get("app_acl_group ag");
			// debugCode($menu->result());
	...
	function _set_own_link(){
		...
		if(	isset($this->jCfg['access']['index']) && count((array) $this->jCfg['access']['index']) > 0){
			$tlink = $this->jCfg['access']['index']->acc_group_controller."/".$this->_getClass()."/";
		} else {
      $tlink = 'meme/' . $this->_getClass();
    }
	...
	function print_preview(){
		...
		//$date = $this->input->get('save')==1?date('Y-m-d H:i:s'):_decrypt(dbClean(trim($this->input->get('dt'))));
		
		$date = _decrypt(dbClean(trim($this->input->get('dt'))));
		$planid = _decrypt(dbClean(trim($this->input->get('pid'))));
		$memberid = _decrypt(dbClean(trim($this->input->get('mid'))));
		$clientid = _decrypt(dbClean(trim($this->input->get('cid'))));
		$relationtype = _decrypt(dbClean(trim($this->input->get('rt'))));
		$roomclass = _decrypt(dbClean(trim($this->input->get('rc'))));
		$servicetype = dbClean(trim($this->input->get('st')));
		$param = array('benefit_serviceid' => $servicetype);
		if(!empty($planid)) $param['benefit_planid'] = $planid;
		
		$data = array(
			'save'			=> $this->input->get('save'),
			'claim'			=> get_name('sos_ref_service_type','service_type',array('service_id' => $servicetype)),
			'date'			=> $date,
			'client'		=> get_name('sos_client','client_name',array('client_id' => $clientid)),
			'member'		=> $this->db->get_where('sos_member',array('member_id' => $memberid))->row(),
			'benefit'		=> $this->db->get_where('sos_ref_benefit',$param)->row()->benefit_description,
			'param'			=> array(								
								'date'			=> $date,
								'planid'		=> $planid,
								'memberid'		=> $memberid,
		...
		$this->_v("print-preview",$data,false);
	}
	
	function print_preview2(){

		//$date = $this->input->get('save')==1?date('Y-m-d H:i:s'):(dbClean(trim($this->input->get('dt'))));
		$date = (dbClean(trim($this->input->get('dt'))));
		$planid = (dbClean(trim($this->input->get('pid'))));
		$memberid = (dbClean(trim($this->input->get('mid'))));
		$clientid = (dbClean(trim($this->input->get('cid'))));
		$relationtype = (dbClean(trim($this->input->get('rt'))));
		$roomclass = (dbClean(trim($this->input->get('rc'))));
		$servicetype = dbClean(trim($this->input->get('st')));
		$param = array('benefit_serviceid' => $servicetype);
		if(!empty($planid)) $param['benefit_planid'] = $planid;
	...
	function print_preview3(){

		//$date = $this->input->get('save')==1?date('Y-m-d H:i:s'):_decrypt(dbClean(trim($this->input->get('dt'))));
		
		$date = dbClean(trim($this->input->get('dt')));
		$desc = dbClean(trim($this->input->get('dk')));
		$planid = _decrypt(dbClean(trim($this->input->get('pid'))));
		$memberid = _decrypt(dbClean(trim($this->input->get('mid'))));
		$clientid = _decrypt(dbClean(trim($this->input->get('cid'))));
		$relationtype = _decrypt(dbClean(trim($this->input->get('rt'))));
		...
		$param = array('benefit_serviceid' => $servicetype);
		if(!empty($planid)) $param['benefit_planid'] = $planid;
		$benefit = $this->db->get_where('sos_ref_benefit',$param)->row();
		...
		$data = array(
			...
			'client'		=> get_name('sos_client','client_name',array('client_id' => $clientid)),
			'member'		=> $this->db->get_where('sos_member',array('member_id' => $memberid))->row(),
			'benefit'		=> !empty($benefit) > 0 ? $benefit->benefit_description:'',
			'param'			=> array(								
								'date'			=> $date,
								'desc'			=> $desc,
								'planid'		=> $planid,
	...
	function pdf_preview(){
		$file = _decrypt(dbClean(trim($this->input->get('fl'))));
		
		$this->_v("viewpdf/index",array('file' => $file),false);
	}
}
