...
class Import extends FrontController {
    ...
    function get_group_id($cid="",$name=""){
    	$groupid = array();
    	$arrname = explode(',',$name);    	
    	for($i=0; $i<count($arrname); $i++) {
		    $cek = $this->db->get_where("sos_tref_provider_group",array('provider_clientid' => $cid, "UPPER(provider_group)" => strtoupper(trim($arrname[$i]))))->row();
        ...
        if( trim($this->input->post('key')) == trim($key) ){
            ...
            if( $cek->num_rows() == 0 ){
                ...
                if($m){
                	...
                	//update.....
	                $group_id = $this->get_group_id($this->input->post('cid'),$this->input->post('group'));
