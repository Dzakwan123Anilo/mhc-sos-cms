...
class News extends AdminController {  
	function __construct()    
	{
		...
		$this->_set_title( 'Master Berita' );
		$this->DATA->table="sos_news";
		$this->folder_view = "master/";
		$this->prefix_view = strtolower($this->_getClass());
		$this->upload_path = cfg('upload_path_photo');
	...
	function delete($id=''){
		...
		if(trim($id) != ''){
			$o = $this->db->delete('sos_news', array("news_id" => idClean($id)));
						
		}
		redirect($this->own_link."/?msg=".urldecode('Delete data Berita success')."&type_msg=success");
	}

	function save(){
		$site_id = $this->input->post('news_site_id');
		...
		$this->upload_max_width = 820;
		$this->upload_max_height = 320;
		$img = $this->_uploaded(
			array(
				'id'		=> $id ,
				'input'		=> 'news_image',
		...
		$msg = !empty($img) || !empty($pdf)?(!empty($img) && !empty($pdf)?'Image : '.$img.'<br>PDF : '.$pdf:(!empty($img)?$img:$pdf)):'Save data Berita success';
		$msgtype = !empty($img) || !empty($pdf)?'warning':'success';
