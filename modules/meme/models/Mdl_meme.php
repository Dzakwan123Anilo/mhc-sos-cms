...
class Mdl_meme extends CI_Model{ 
	...
	function data_user($p=array(),$count=FALSE){
		$total = 0;
		/* table conditions */
		$this->db->select('app_user.*,app_site.site_url,app_site.site_name');
		$this->db->join("app_site","app_site.site_id = app_user.user_site_id");
		...
		if( $this->jCfg['user']['is_all'] != 1 ){
			$this->db->where("user_site_id",$this->jCfg['user']['site']['id']);
		}
...
//			$this->db->order_by('user_id','DESC');
	...
	function data_log($p=array(),$count=FALSE){
		...
		$this->db->select("app_log.*,app_acl_accesses.acc_menu");
		$this->db->join("app_acl_accesses","app_acl_accesses.acc_controller_name = app_log.log_class","LEFT");
		...
		$this->db->where("app_log.log_class !=","data");
		$this->db->where("app_log.log_user_name !=","none");
