...
class Group_provider extends AdminController {  
	...
	function edit($id=''){
		...
		if(trim($id)!=''){
			$row = $this->DATA->data_id(array(
					'providergroupId'	=> $id
				));
			$this->data_form = $row;
			
			$this->db->join('sos_tref_member_provider_group','sos_tref_member_provider_group.providerId=sos_provider.provider_id AND sos_tref_member_provider_group.providergroupId = '.$id,'left');
			if(!empty($row->provider_clientid))
				$this->db->where('sos_provider.provider_clientid', $row->provider_clientid);
			$this->db->order_by('sos_provider.provider_name','ASC');
			$data['provider'] = $this->db->get('sos_provider')->result();
	...
	function save(){
        ...
        if( isset($_POST['item_select']) && count($_POST['item_select'])>0 ){
            foreach ((array)$this->input->post('item_select') as $key => $value) {
	...
	function get_data_provider(){
		...
		if( trim($id)!="" ){
			...
			$tmp_provider = $this->db->get_where('sos_provider',  array('provider_clientid' => $id))->result();
			$data['provider'] = $tmp_provider;
