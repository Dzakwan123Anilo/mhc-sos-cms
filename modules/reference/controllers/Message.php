...
class Message extends AdminController {  
	function __construct()    
	{
		...
		$this->_set_title( 'Referensi Message Email' );
		$this->DATA->table="sos_ref_message_email";
		...
		$this->cat_search = array(
			...
			'message_content'	=> 'Konten Message'
		); 
	...
	function index(){
		...
		$this->data_table = $this->M->message($par_filter);
	...
	function delete($id=''){
		...
		if(trim($id) != ''){
			$o = $this->db->delete('sos_ref_message_email', array("message_id" => idClean($id))
			);
						
		}
		redirect($this->own_link."/?msg=".urldecode('Delete data Message Email success')."&type_msg=success");
	}
	...
	function save(){
		...
		redirect($this->own_link."?msg=".urldecode('Save data Message Email success')."&type_msg=success");
	}
